...
2. 「録画」タブの上のほう、**「チェックして取り込む」** を押す
3. 新しい録画があれば、文字起こし → 議事録 → LINEに「🗒 議事録ができました」まで進みます
4. スマホで `meetings.html` を開けば読めます

この議事録は無料の軽いモデル（Gemini）が作るので、要点はつかめますが浅めです。
**大事な会議は、下の A でClaudeに作り直してもらってください**（同じ会議を上書きします）。

> 以前は10分おきに自動チェックしていましたが、会議の頻度が低いので手動ボタンにしました。
> また自動に戻したいときは、PowerShell で
> `Enable-ScheduledTask -TaskName '会議の文字起こし（監視）'`。
...
録画の **「クラウドに送る」** を押すと、その場で Gemini が議事録にします（自動と同じ品質）。

| タブ | できること |
|---|---|
| **会議** | 会議ごとに議事録を開く。アクション表・議題・その他・録画で確認すべき箇所 |
| **アクション** | 全会議を横断して、期限が近い順に並べる。期限切れは赤、3日以内は黄 |
| **検索** | 過去の議事録を横断して探す（スペース区切りでAND） |

各議事録の下に **「この議事録を削除」** ボタンがあります。作り直しに失敗したときなどに。
同じ録画をもう一度送ると議事録は**上書き**されます。
