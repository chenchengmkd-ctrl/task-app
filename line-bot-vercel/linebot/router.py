...
def route_command(user_id, text):
    ...
    # 入力確認（旧・毎日16時の自動リマインド。プッシュ通知削減のため自動送信は廃止し、見たい時に呼ぶ形に変更）
    if re.match(r'^(入力確認|入力リマインド)$', text):
        from . import finance as finance_mod
        return finance_mod.build_finance_reminder()
...
def help_text():
    lines = [
        ...
        '・月〜土 11:30/12:00/12:30/13:00：その時点の売上速報（日曜は無し）',
        f'・時刻つきタスクは開始{config.TIME_LEAD_MINUTES}分前にもリマインド',
        '',
        '（毎朝の期限リマインド・振り返りの催促・定期タスクの通知・日報・期限未設定タスクの棚卸し・'
        '毎日16時の入力リマインドは、LINEの月間無料通数を使い切ったこともあり自動送信を廃止しました。'
        '「通知」「振り返り」「定期タスク」「日報」「入力確認」と送れば今でも見られます）',
    ]
    return '\n'.join(lines)
