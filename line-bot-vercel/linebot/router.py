...
def help_text():
    lines = [
        ...
        '・今日の予定を決める → 時間帯に関係なく、必ず今日ぶんのやる候補を番号つきで表示（「1,3,5」と番号を返して確定）',
        '・予定を決める（＝明日の予定）→ やる候補を番号つきで表示（「1,3,5」と番号を返して確定）',
        '　（20時より前に送るとその日ぶん、20時以降に送ると翌日ぶんの予定になります）',
        '・今日の予定 → 決めたぶんが今どこまで進んでいるか（済み／未済）を番号つきで表示',
        ...
        '・テンプレ → 記入用のひな形が届きます。コピーして数字を入れ、',
        '　そのまま送り返せば複数行まとめて登録できます（この方法が一番かんたん）',
        '・1行ずつ送ってもOKです（金額は税抜。売上だけ税込）',
        '　例）売上 52000',
