...
def looks_like_date_reply(t):
    """日付の返信らしさを判定（日付らしい表現を含むか）。含まなければ別の話題（＝新しいタスク等）とみなしてよい。

    以前は数字が1つでも入っていれば日付の返信とみなしていたが、「100円」「20枚」のように
    数字を含むだけの新しいタスクまで巻き込んで「日付を認識できませんでした」と聞き返してしまい、
    タスクとして追加されない不具合があった（2026-08-26）。日付らしい形（M/D・曜日・相対表現）に絞る。
    """
    return bool(re.search(
        r'[0-9０-９]{1,2}\s*[/月]\s*[0-9０-９]{1,2}|今日|明日|明後日|来週|再来週|今週|来月|今月|月末|週末|'
...
def looks_like_time_reply(t):
    """時刻の返信らしさを判定（時刻らしい表現を含むか）。含まなければ別の話題（＝新しいタスク等）とみなしてよい。

    以前は数字が1つでも入っていれば時刻の返信とみなしていたため、looks_like_date_reply と同様の
    不具合があった（2026-08-26）。時刻を表す区切り（:・：・時）を伴う数字に絞る。
    """
    return bool(re.search(r'[0-9０-９]{1,2}\s*[:：時]|正午|午前|午後', t))
